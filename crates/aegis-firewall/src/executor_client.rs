...
impl ExecutorClient {
    ...
    pub async fn execute(&self, request: ExecRequest) -> Result<ExecResponse, AegisError> {
        ...
        let mut payload = serde_json::to_string(&request).map_err(|e| {
            AegisError::Internal(format!("Lỗi serialize ExecRequest: {e}"))
        })?;
        payload.push('\n');

        // 3. Gửi payload qua socket
        stream.write_all(payload.as_bytes()).await.map_err(|e| {
            AegisError::Internal(format!("Lỗi ghi dữ liệu tới Execd socket: {e}"))
        })?;
        stream.flush().await.map_err(|e| {
            AegisError::Internal(format!("Lỗi flush Execd stream: {e}"))
        })?;
        ...
        buf_reader.read_line(&mut response_line).await.map_err(|e| {
            AegisError::Internal(format!("Lỗi đọc kết quả từ Execd socket: {e}"))
        })?;

        // 5. Deserialize JSON thành ExecResponse
        let response: ExecResponse = serde_json::from_str(&response_line).map_err(|e| {
            AegisError::Internal(format!("Lỗi deserialize ExecResponse: {e}"))
        })?;
