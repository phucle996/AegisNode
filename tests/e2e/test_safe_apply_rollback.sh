...
sudo ./scripts/install.sh --version v1.1.1 >/dev/null 2>&1 || true
...
if [[ "$STATUS_RES" =~ "online" ]] || [[ "$STATUS_RES" =~ "RUNNING" ]]; then
...
if [[ "$APPLY_RES" =~ "execution_id" ]] || [[ "$APPLY_RES" =~ "pending_confirmation" ]] || [[ "$APPLY_RES" =~ "executionId" ]]; then
